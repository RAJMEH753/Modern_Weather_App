...
if(x == "01"){
	weather[0] = { type: 'sun', name: 'Sunny'};
}else if(x == 13){
	weather[0] = { type: 'snow', name: 'Snow'};
}else if(x == 11){
	weather[0] = { type: 'thunder', name: 'Storms'};
}else if(x == "09" || x == 10){
	weather[0] = { type: 'rain', name: 'Rain'};
}else if(x == "02" || x == "03"){
	weather[0] = { type: 'wind', name: 'Windy'};
}else if(x == "04"){
	weather[0] = { type: 'cloudy', name: 'Dark'};
}else{
	weather[0] = { type: 'haze', name: 'Fog'};
}
console.log(x);
// 🛠 app settings
...
function makeRain() {
	...
	var lineLength = currentWeather.type == "thunder" || "cloudy" ? 35 : 14;
	// var lineLength = currentWeather.type == "cloudy" ? 35 : 14;
	...
	var line = this["innerRainHolder" + (3 - Math.floor(lineWidth))]
		...
		.attr({
			fill: "none",
			stroke: currentWeather.type == "thunder" || "cloudy" ? "#777" : "#0000ff",
			// stroke: currentWeather.type == "cloudy" ? "#777" : "#0000ff",
...
function makeSplash(x, type)
{
	...
	var splashLength = type == "thunder" || "cloudy" ? 30 : 20;
	// var splashLength = type == 'cloudy' ? 30 : 20;
	
	// splashBounce is the max height the line will curve up
	// before falling
	var splashBounce = type == "thunder" || "cloudy" ? 120 : 100;
	// var splashBounce = type == 'cloudy' ? 120 : 100;
	...
	// the same for both
	var speed = type == "thunder" || "cloudy" ? 0.7 : 0.5;
	// var speed = type == 'cloudy' ? 0.7 : 0.5;
	...
	var splash = outerSplashHolder.path(points.join(' ')).attr({
      	fill: "none",
      	stroke: type == "thunder" || "cloudy" ? '#777' : '#0000ff',
		// stroke: type == 'cloudy' ? '#777' : '#0000ff',
      	strokeWidth: 1
    });
...
function startLightningTimer()
{
	...
	if(currentWeather.type == 'thunder' || currentWeather.type == 'cloudy')
	{
...
function changeWeather(weather)
{
	...
	switch(weather.type)
	{
		...
		case 'sun':
			...
			break;
		case 'haze':
			TweenMax.to(settings, 3, {windSpeed: 0.2, ease: Power2.easeInOut});
			break;
		default:
	...
	{
		case 'sun':
			TweenMax.to(sun.node, 4, {x: sizes.card.width / 2, y: sizes.card.height / 3, ease: Power2.easeInOut});
